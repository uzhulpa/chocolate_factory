...
public partial class SaveItemFormControl : ContentView
{
	...
	private async Task SaveItemAsync()
	{
        ...
        {
            await ErrorAlertAsync("Неверно заполнено поле пищевой ценности: белки на 100 г.");
            return;
        }

        if (Item.NutritionalInfo.ProteinsPer100g < 0)
        {
            await ErrorAlertAsync("Неверно заполнено поле пищевой ценности: жиры на 100 г.");
            return;
        }
        if (Item.NutritionalInfo.ProteinsPer100g < 0)
        {
            await ErrorAlertAsync("Неверно заполнено поле пищевой ценности: углеводы на 100 г.");
            return;
        }
